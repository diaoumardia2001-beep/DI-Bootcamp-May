...
mot_user = input("Please enter a word: ")
...
letter_index_dict = {}
for index, letter in enumerate(mot_user):
...
if not basket:    print("Nothing")
else:    print(sorted(basket))
#4. Examples:
#Given:
#items_purchase = {"Water": "$1", "Bread": "$3", "TV": "$1,000", "Fertilizer": "$20"}
#wallet = "$300"
define_items_purchase = {"Water": "$1", "Bread": "$3", "TV": "$1,000", "Fertilizer": "$20"}
define_wallet = "$300"
basket = []
wallet_amount = int(define_wallet.replace("$", "").replace(",", ""))
for item, price in define_items_purchase.items():
    price_amount = int(price.replace("$", "").replace(",", ""))
    if price_amount <= wallet_amount:
        basket.append(item)
        wallet_amount -= price_amount
...
#wallet = "$100"
define_items_purchase = {"Apple": "$4", "Honey": "$3", "Fan": "$14", "Bananas": "$4", "Pan": "$100", "Spoon": "$2"}
define_wallet = "$100"
basket = []
wallet_amount = int(define_wallet.replace("$", "").replace(",", ""))
for item, price in define_items_purchase.items():
...
#wallet = "$1"
define_items_purchase = {"Phone": "$999", "Speakers": "$300", "Laptop": "$5,000", "PC": "$1200"}
define_wallet = "$1"
basket = []
wallet_amount = int(define_wallet.replace("$", "").replace(",", ""))
for item, price in define_items_purchase.items():
